...
<div class="form-row">
  ...
  <input type="number"
         ...
         data-ordered="<?= (int)$order['order_quantity'] ?>"
         required>
</div>

<p id="qtyWarning"
   style="display:none;color:#d10000;font-weight:bold;">
  ※ 発注数量と入荷数量が一致していません
</p>
...
const ordered = Number(qty.dataset.ordered);
const warn = document.getElementById('qtyWarning');

qty.addEventListener('input', () => {
  warn.style.display = (Number(qty.value) !== ordered) ? 'block' : 'none';
});
</script>
